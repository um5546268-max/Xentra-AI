        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-slate-500">
              Open a page, or chain multiple steps together.
            </p>
          </div>
        </div>
...
function SingleMode() {
  ...
  const handleOpen = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(
        err?.response?.data?.detail || err.message || "Failed to open URL"
      );
    } finally {
      setLoading(false);
    }
      ...
      {result && (
        <div className="space-y-4">
          ...
          <button
            ...
            disabled={sending || sent || !result.text}
            className={`w-full rounded-xl border px-4 py-3 text-sm font-medium transition flex items-center justify-center gap-2 ${
              sent
                ? "border-emerald-500 bg-emerald-500/10 text-emerald-300"
                : "border-slate-700 bg-slate-900/60 text-slate-200 hover:border-violet-500 hover:bg-slate-900 hover:text-violet-300"
            }`}
          >
            {sent ? (
              <>
                <Check className="w-4 h-4" />
                Conversation created — opening…
              </>
            ) : sending ? (
              <>
                <Loader2 className="w-4 h-4 animate-spin" />
              ...
              <>
                ...
                Send to chat and ask about this page
              </>
            )}
          </button>
      ...
      {!result && !error && !loading && (
        <div className="text-center py-16 space-y-2 text-slate-600">
          ...
          <div className="text-sm">
            Enter a URL to open it in the browser agent.
          </div>
        </div>
      )}
    </div>
...
function ChainMode() {
  ...
  const runChain = async () => {
    ...
    try {
      ...
      setError(
        err?.response?.data?.detail || err.message || "Chain failed"
      );
    } finally {
      setLoading(false);
    }
      ...
      <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-4 space-y-3">
        ...
        <div className="space-y-2">
          {steps.map((step, i) => (
            <div
              key={i}
              className="flex items-start gap-2 rounded-lg border border-slate-800 bg-slate-950/60 p-2.5"
            >
              <div className="w-6 h-6 rounded bg-slate-800 flex items-center justify-center text-xs font-mono text-slate-400 shrink-0">
                {i + 1}
              </div>
              ...
              <div className="flex-1 space-y-1.5">
                ...
                {step.action === "open" && (
                  <input
                    ...
                    className="w-full rounded border border-slate-700 bg-slate-900 px-2 py-1 text-xs text-white focus:border-violet-500 focus:outline-none"
                  />
                )}
                ...
                {step.action === "click" && (
                  <input
                    ...
                    className="w-full rounded border border-slate-700 bg-slate-900 px-2 py-1 text-xs text-white font-mono focus:border-violet-500 focus:outline-none"
                  />
                )}
                ...
                {step.action === "fill" && (
                  <div className="grid grid-cols-2 gap-1.5">
                    <input
                      ...
                      className="rounded border border-slate-700 bg-slate-900 px-2 py-1 text-xs text-white font-mono focus:border-violet-500 focus:outline-none"
                    />
                    <input
                      value={step.value || ""}
                      onChange={(e) => updateStep(i, { value: e.target.value })}
                      placeholder="value"
                      className="rounded border border-slate-700 bg-slate-900 px-2 py-1 text-xs text-white focus:border-violet-500 focus:outline-none"
                    />
                  </div>
                )}
                ...
                {step.action === "wait" && (
                  <input
                    ...
                    value={step.ms || 1000}
                    onChange={(e) =>
                      updateStep(i, { ms: parseInt(e.target.value) || 1000 })
                    }
                    placeholder="milliseconds"
                    className="w-32 rounded border border-slate-700 bg-slate-900 px-2 py-1 text-xs text-white focus:border-violet-500 focus:outline-none"
                  />
                )}
              </div>
        ...
        <button
          ...
          className="w-full rounded-lg bg-violet-600 px-4 py-2.5 text-sm font-medium hover:bg-violet-500 disabled:opacity-40 transition flex items-center justify-center gap-2"
        >
          {loading ? (
            <>
              <Loader2 className="w-4 h-4 animate-spin" />
        ...
        <div className="space-y-3">
          <div className="rounded-2xl border border-emerald-500/40 bg-emerald-500/5 p-4">
            <div className="flex items-center gap-2 text-sm text-emerald-300">
              <Check className="w-4 h-4" />
              Chain completed · {result.steps.length} steps
            </div>
            <div className="mt-2 text-xs text-slate-500">Final URL</div>
            <div className="font-mono text-xs text-slate-300 truncate">
              {result.final_url}
            </div>
            <div className="mt-1 text-xs text-slate-500">Final title</div>
            <div className="text-sm text-slate-200">
              {result.final_title || "(no title)"}
            </div>
          </div>
          ...
          {result.steps.map((s) => (
            ...
            >
              ...
              <div className="p-3 space-y-2">
                ...
                {s.text && (
                  <div className="text-xs text-slate-500 max-h-32 overflow-y-auto whitespace-pre-wrap">
                    {s.text.slice(0, 500)}…
                  </div>
                )}
              </div>
...
function SmallAddBtn({
  ...
  return (
    <button
      onClick={onClick}
      className="flex items-center gap-1 rounded-md border border-slate-700 bg-slate-900 px-2 py-1 text-xs text-slate-400 hover:text-violet-300 hover:border-violet-500 transition"
    >
