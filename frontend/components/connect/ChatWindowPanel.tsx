...
export default function ChatWindowPanel({ chatId }: { chatId: string }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 sm:gap-3 p-2.5 sm:p-4 border-b border-slate-800 bg-slate-950/95 backdrop-blur-lg">
        {/* Back */}
        <button
          onClick={() => {
            window.location.href = "/app/connect";
        ...
        <div className="relative shrink-0">
          <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-gradient-to-br from-violet-500 to-cyan-500 flex items-center justify-center text-white font-semibold text-sm">
            {initial}
          </div>
          <div className="absolute bottom-0 right-0 w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full bg-emerald-500 border-2 border-slate-950" />
        </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-[10px] sm:text-xs text-emerald-400 truncate">
            ...
            <span className="sm:hidden">
              {chat.type === "direct" ? "Online" : statusText}
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-0.5 sm:gap-1">
          ...
          {/* Ask Xentra — compact icon on mobile, full on desktop */}
          <button
            onClick={() => setShowAskXentra(true)}
            className="hidden sm:flex items-center gap-1.5 rounded-lg border border-violet-500/40 bg-violet-500/10 px-3 py-1.5 text-xs font-medium text-violet-300 hover:bg-violet-500/20 transition"
            title="Ask Xentra"
          >
          ...
          <button
            onClick={() => setShowAskXentra(true)}
            className="sm:hidden w-8 h-8 rounded-lg flex items-center justify-center text-violet-300 hover:bg-violet-500/20 transition"
            title="Ask Xentra"
          >
      ...
      <div
        className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 bg-cover bg-center bg-no-repeat"
        style={{
          backgroundImage: (() => {
        ...
        <div className="flex items-center gap-1 sm:gap-2">
          ...
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full rounded-full border border-slate-800 bg-slate-900 px-3 sm:px-4 py-2 sm:py-2.5 text-sm placeholder-slate-600 focus:border-violet-500 focus:outline-none"
            />
          </div>
          ...
          <button
            ...
            className="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-gradient-to-br from-violet-600 to-cyan-600 flex items-center justify-center text-white disabled:opacity-40 hover:opacity-90 transition shrink-0"
          >
            {sending ? (
...
function IconButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      title={label}
      className="w-8 h-8 sm:w-9 sm:h-9 rounded-full flex items-center justify-center text-slate-400 hover:bg-slate-800 hover:text-white transition shrink-0"
    >
      {icon}
    </button>
  );
    ...
    >
      <div className="max-w-[85%] sm:max-w-[70%] relative group">
        <div
          ...
          className={`rounded-2xl px-3.5 sm:px-4 py-2.5 ${
