...
import { Splitter } from "@/components/code/Splitter";
...
export default function DesktopCode() {
  ...
  const handleDebug = async () => {
    if (!activePath) {
      appendTerminal("[warn] No file open to debug");
      setTerminalTab("terminal");
      return;
    }
    setTerminalTab("debug");
    appendTerminal(`$ debug ${activePath}`);

    try {
      const res = await debugFile(activePath, []);
      setRunOutput(res);
      if (res.stdout) appendTerminal(res.stdout);
      if (res.stderr) appendTerminal(res.stderr);
      if (res.hint) appendTerminal(`[info] ${res.hint}`);
      appendTerminal(`[info] exit ${res.exit_code}`);
    } catch (err: any) {
      appendTerminal(`[error] ${err?.message ?? "debug failed"}`);
    }
  };
  ...
  const handleTest = async () => {
    ...
    try {
      const res = await runTests(activePath ?? undefined);
      setRunOutput(res);
      if (res.stdout) appendTerminal(res.stdout);
      if (res.stderr) appendTerminal(res.stderr);
      if (res.summary) {
        appendTerminal(
          `[info] ${res.summary.passed} passed, ${res.summary.failed} failed, ${res.summary.skipped} skipped`
        );
      }
      appendTerminal(`[info] exit ${res.exit_code}`);
    } catch (err: any) {
      appendTerminal(`[error] ${err?.message ?? "test failed"}`);
    }
  };
  ...
  const resizeAiPanel = (delta: number) => {
    setAiPanelWidth((w) => Math.max(240, Math.min(600, w - delta)));
  };
  const resizePreview = (delta: number) => {
    setPreviewWidth((w) => Math.max(280, Math.min(900, w - delta)));
  };
  ...
  return (
    <div className="h-full flex flex-col bg-slate-950 text-slate-100 overflow-hidden">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        {/* Explorer ↔ Editor splitter */}
        <Splitter orientation="vertical" onResize={resizeExplorer} />
        ...
        {/* Editor ↔ Live Preview splitter */}
        {previewOpen && (
          <>
            <Splitter orientation="vertical" onResize={resizePreview} />
            <div style={{ width: previewWidth }} className="shrink-0 h-full">
        ...
        {aiPanelOpen && (
          <>
            <Splitter orientation="vertical" onResize={resizeAiPanel} />
            <div style={{ width: aiPanelWidth }} className="shrink-0 h-full">
              <AIAgentPanel
