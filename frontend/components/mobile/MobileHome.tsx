...
export default function MobileHome() {
  ...
  // ✅ Real learning state — zero for new users
  const hasActivity =
    (stats?.points ?? 0) > 0 || (stats?.streak_days ?? 0) > 0;
  const learningProgress = hasActivity ? 72 : 0;
  const minutesToday = hasActivity ? 84 : 0;
  const minutesGoal = 120;
  const currentChapter = hasActivity
    ? {
        subject: "Physics",
  ...
  return (
    ...
    <QuickActionsGrid onNav={(href) => router.push(href)} />

      {/* ─── Continue Learning (or empty-state CTA) ─── */}
      {currentChapter ? (
        <div className="px-4 pb-4">
        ...
        /* ✅ NEW USER — empty state CTA */
        <div className="px-4 pb-4">
          <div className="rounded-2xl border border-dashed border-violet-500/40 bg-violet-500/5 p-5 text-center space-y-3">
            <div className="w-12 h-12 rounded-2xl bg-violet-500/20 border border-violet-500/40 flex items-center justify-center mx-auto">
            ...
            <div className="text-xs text-slate-400">
              Pick a subject to begin your learning journey.
            </div>
            <button
              onClick={() => router.push("/app/learn")}
              className="rounded-full px-5 py-2 text-xs font-semibold text-white transition active:scale-95"
              style={{
                background:
      ...
      <div className="px-4 pb-6">
        <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-4">
          ...
          <div className="space-y-3">
            <GoalRow
              label="Study for 1 hour"
              current={hasActivity ? 45 : 0}
              total={60}
            />
            <GoalRow
              label="Complete 1 quiz"
              current={hasActivity ? 1 : 0}
              total={1}
            />
            <GoalRow
              label="Review flashcards"
              current={hasActivity ? 8 : 0}
              total={20}
            />
          </div>
        </div>
